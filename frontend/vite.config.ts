...
/**
 * Every variable a production bundle cannot work without.
 *
 * These are read at build time and baked into the output, so a missing one is
 * not something the app can recover from later — and each fails in a way that
 * is hard to read from the browser. `VITE_API_URL` is the worst of them: unset,
 * the client falls back to localhost and a real user sees connection errors
 * against their own machine. Failing the build is the only place this is cheap
 * to fix.
 */
    ...
    if (missing.length > 0) {
      throw new Error(
        `Cannot build: ${missing.join(", ")} not set. ` +
          `Set them in the Vercel project's environment variables (or .env for a local build).`,
      );
    }
  }
